...
## 14. Опциональный LLM-помощник

LLM подключается **после** детерминированного решения и только для безопасного текстового пояснения `DENY` / `MANUAL_REVIEW` сотруднику:

```text
decision engine → CLOSED + audit event → optional LLM explainer → понятное пояснение сотруднику
```

Этот контур асинхронный и не имеет интерфейса к турникету. Внешний API может быть недоступен без влияния на распознавание, решение, audit и физическую команду. В LLM передаётся только минимальный обезличенный безопасный контекст причины и рекомендуемого действия; внутренние reason-коды, биометрические данные и идентификатор сотрудника не отправляются.
...
2. **Детерминированный критический путь:** LLM допускается только как post-decision explainer без доступа к турникету
