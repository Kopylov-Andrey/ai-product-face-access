...
**LLM НЕ используется для обнаружения, сопоставления, liveness или принятия решения `ALLOW/DENY/MANUAL_REVIEW`.** Критический путь полностью детерминирован.

В PoC реализован только **опциональный post-decision explainer** для `DENY` и `MANUAL_REVIEW`: после фиксации решения он получает минимальный обезличенный контекст audit-события и формирует 2 коротких предложения для сотрудника: понятную причину и безопасное следующее действие. У модуля нет доступа к `TurnstileSimulator`, `command_id` или функции принятия решения.

**Почему LLM вынесена за критический путь:**
1. Недетерминизм несовместим с safety-инвариантами физического доступа.
2. Внешняя API-зависимость не должна влиять на p95 критического пути.
3. Служебное расследование требует воспроизводимого исходного решения.
4. Ошибка/timeout LLM должны быть безопасно игнорируемы.

В LLM не передаются изображения, `employee_id`, embedding, match-score, пороги и top-K кандидаты. Детали интеграции: [`docs/llm.md`](llm.md).
...
10. **LLM только после решения:** критический путь остаётся детерминированным, аудируемым и независимым от внешней модели
