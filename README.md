...
Локальная панель охраны использует **тот же `run_demo` и decision engine**, что CLI и тесты; отдельной логики допуска в UI нет.
...
Пример risky-сценария: неоднозначное совпадение переводится в ручную проверку, турникет остаётся `CLOSED`, а опциональная LLM формирует только постфактум понятное сотруднику пояснение.

![Web-MVP: ручная проверка при неоднозначном совпадении](docs/assets/web-mvp.png)

### Опциональное LLM-пояснение для сотрудника

Для `DENY` и `MANUAL_REVIEW` web-MVP может сформировать для сотрудника короткое понятное пояснение: почему проход сейчас не выполнен и что безопасно сделать дальше. LLM **не участвует в идентификации и не может изменить `CLOSED`, `ALLOW/DENY/MANUAL_REVIEW` или сформировать `command_id`**. Во внешний сервис передаётся только обезличенный безопасный контекст без изображения лица, `employee_id`, embedding, match-score, порогов, top-K кандидатов и внутренних reason-кодов.
...
Если ключ или провайдер недоступны, основной PoC и панель контроля доступа продолжают работать без LLM. Подробности: [контракт LLM-помощника](docs/llm.md).
...
| LLM | **опциональное пояснение сотруднику после DENY/MANUAL_REVIEW** | пользовательское объяснение без права менять access decision или физическую команду |
...
- [`docs/llm.md`](docs/llm.md) — контракт опционального LLM-пояснения сотруднику и границы безопасности
